...
function tests:cov {
    ...
    TEST_FILE="${1:-$(get:python:files:tests)}"
    shift || true
    poetry run pytest "$TEST_FILE" --cov=test_project  --cov-report=term "$@"
}
...
function docs:api {
    ...
    cd docs && poetry run sphinx-apidoc -o api ../src/test_project -f
}
...
function help {
    echo "$0 <task> <args>"
    echo ""
    echo "====== test-project Development Tool ======"
    echo ""
    echo "Environment:"
